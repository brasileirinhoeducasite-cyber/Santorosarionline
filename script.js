...
function apagarTela() {
  ...
  btnMenu.style.display = 'block'; // mostra o botão menu
}

// Função para mostrar a tela inicial (menu)
function mostrarMenuInicial() {
  document.getElementById('botoes').style.display = 'flex';
  document.querySelector('header').style.display = 'block';
  document.getElementById('container').innerHTML = '';
  document.getElementById('divBolas').style.display = 'none';
  document.getElementById('tresmarias').style.display = 'none';
  btnMenu.style.display = 'none'; // esconde o botão menu na tela inicial
  c = 0; // reset contador bolas
  // Remove listener para evitar clique duplo
  window.removeEventListener('click', Clique);
}

// Pega o botão Menu
const btnMenu = document.getElementById('btnMenu');

// No clique do botão Menu volta ao menu inicial
...
var contador;
var Clique; // vamos definir fora pra poder remover
...
function executar(a) {
  ...
  // Remove listener anterior antes de adicionar para evitar multiplos listeners
  window.removeEventListener('click', Clique);
  ...
  Clique = function () {
    ...
    if (contador > 2 && contador < 6) {
      if (a[contador - 1] === aveMaria) {
        document.querySelectorAll('div .tresmarias')[c - 1].style.backgroundColor = 'rgba(107, 159, 207, 1)';
      }
      tremarias.style.display = "flex";
      var verm = document.querySelectorAll('div .tresmarias')[c];
      verm.style.backgroundColor = 'rgba(94, 170, 241, 1)';
      c++;
    } else {
      tremarias.style.display = "none";
      if (a[contador] === aveMaria) {
        if (a[contador - 1] === aveMaria) {
          document.querySelectorAll('li div')[c - 1].style.backgroundColor = 'rgba(107, 159, 207, 1)';
        }
        divBolas.style.display = "flex";
        var red = document.querySelectorAll('li div')[c];
        red.style.backgroundColor = 'rgba(94, 170, 241, 1)';
        c++;
      } else {
        if (a[contador - 1] === aveMaria) {
          document.querySelectorAll('li div')[c - 1].style.backgroundColor = 'rgba(107, 159, 207, 1)';
